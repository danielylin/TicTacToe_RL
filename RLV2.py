...
class RL:
    ...
    def q_learning(self,
        ...
        # select_action = lambda state, Q, epsilon: np.argmax(Q[state, legal_actions]) \
        #     if np.random.random() > epsilon \
    ...
    def select_action(self, state, Q, epsilon):
        ...
        if np.random.random() > epsilon:
            return np.argmax(Q[state, legal_actions])
        else:
            return random.choice(legal_actions)

    def get_legal_actions(self, state):
        board = self._undiscretize_board(state)
        legal_actions = []
        for a in np.arange(0, 9):
            idx = self.convert_action_to_index(a)
            if board[idx] != 0:
                continue
            else:
